...
import fetchLookUpValues from '@salesforce/apex/multiSelectCombo1.fetchLookUpValues';
import getPickListValuesIntoList from '@salesforce/apex/multiSelectCombo1.getPickListValuesIntoList';


export default class ComboBoxNew extends LightningElement {
    @track options = [];
    @api searchKey;
    @track searchValue = '';
    // @api objectName;
    ...
    @wire(getPickListValuesIntoList) picklistvalues1({ data, error }) {
        if (data) {
            let picklistOptions = [{ key: '--None--', value: '--None--' }];
            let i = 0;
            data.forEach(key => {
